...
import { useLocation } from 'react-router-dom'; // Sayfa kontrolü için
...
const Navbar = () => {
  ...
  // Mantık: Navbar ne zaman beyaz zeminli (solid) olmalı?
  // 1. Sayfa aşağı kaydırıldıysa (scrolled)
  // 2. YA DA Ana sayfada değilsek (isHome false ise)
  ...
  const handleLanguageChange = (langCode) => {
    ...
    if (select) {
        ...
        select.dispatchEvent(new Event('change')); // Değişikliği tetikle
    }
};
  ...
  return (
    <nav 
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        isSolid 
          ? "bg-white text-gray-800 shadow-lg py-2"  // DOLU MOD: Beyaz zemin, koyu yazı
          : "bg-transparent text-white py-6"        // ŞEFFAF MOD: Şeffaf zemin, beyaz yazı
      }`}
    >
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
            {/* 2. MİLLİ TEKNOLOJİ HAMLESİ LOGOSU */}
  ...
  <FaSearch className={`cursor-pointer transition-colors ${isSolid ? 'text-gray-400 hover:text-caku-red' : 'text-white hover:text-brand-light'}`} />
        ...
        <div className={`flex items-center gap-2 text-xs font-bold tracking-wider ${isSolid ? 'text-gray-600' : 'text-white'}`}>
            ...
            className="hover:text-caku-red transition-colors"
            >
            TR
            </button>
        ...
        {/* Arka planda çalışan gizli çeviri motorunu buraya koyuyoruz */}
        <GTranslate />
